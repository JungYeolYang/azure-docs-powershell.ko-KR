...
* `Get-AzResource`의 결과를 ??(으)로 파이프할 때 발생하는 문제 수정 `Set-AzResource`
    - 자세한 내용은 여기 https://github.com/Azure/azure-powershell/issues/8240를 참조하세요.
* 실행 시 JSON 데이터 형식 변경 문제 해결 `Set-AzResource`
...
### <a name="azresources"></a>Az.Resources
* `New-AzureRmPolicyAssignment` 및 ??의 리소스 ID 관련 매개 변수에 대한 도움말 문서 업데이트 `Set-AzureRmPolicyAssignment`
